...
public class UserServiceImpl implements UserService {
    ...
    private boolean checkIfUserCanBeDeleted(User user){
        switch (user.getRole().getDescription()){
            case "Manager":
                List<ProjectDTO>projectDtoList=projectService.listAllNonCompleteByAssignedManager(userMapper.convertToDto(user));
            return projectDtoList.size()==0;
                case "Employee":
                    List<ProjectDTO>taskDtoList=projectService.listAllNonCompleteByAssignedEmployee(userMapper.convertToDto(user));
